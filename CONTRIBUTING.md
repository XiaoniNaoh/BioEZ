...
如果没通过，脚本会**按错误类型给出对应的修复建议**，例如：

- 图片路径不对 → 提示「最常见的原因是笔记移到子目录后没改相对路径」，并给出正确写法；
- 缺元数据 / 文件名不合规 → 提示文件名该长什么样，并说明重跑脚本会自动补齐元数据；
- 课程目录改了没同步配置 → 提示去改 `data/course-config.json` 的 `directory` 字段；
- 生成文件过期、图片清单不一致 → 提示重跑哪条命令。
...
> **推荐装一次「提交前检查」**：在仓库根目录双击 **`安装提交前检查.command`**。
> 之后每次 Commit，都会先检查「这次提交里的课程正文」和「课程目录 / 导航 / 课程清单」是否对得上；
> 对不上就拦下提交，并提示你先跑「准备提交」。它只看暂存区（也就是这次提交真正会写入的内容），
> 所以本地还没提交的草稿不会干扰判断，也不会因为草稿而误拦。
> 跳过某一次检查：`git commit --no-verify`；彻底关掉：`git config --unset core.hooksPath`。
...
另外：每门课根目录的 `COURSE_INDEX.md` 是课程入口，**由脚本生成，不要手改**。`09` 目录是暂不发布的模块，先不要往仓库里加。
